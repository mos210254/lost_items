...
const fetchAdmins = async () => {
    try {
        admins.value = await $fetch(`${BASE_URL}/api/admin`);
    } catch (error) {
...
async function add_user() {
    ...
    async function test() {
        ...
        const res = await $fetch(`${BASE_URL}/admin/insert`, {
            method: "POST",
            body: data,
...
async function edit_user(id_user) {
    ...
    const userData = await $fetch(`${BASE_URL}/api/admin`);
    ...
    async function updateUser() {
        ...
        const res = await $fetch(`${BASE_URL}/admin/edit_user`, {
            method: "PUT",
            body: data,
            headers: { "Content-Type": "application/json" },
        });
...
<template>
    <div class="container mx-auto p-6">
        ...
        <div class="w-full">
            ...
            <!-- Mobile Stack Mode -->
            <div class="md:hidden">
                <div v-for="admin in admins" :key="admin.id" class="bg-white shadow-md rounded-lg p-4 mb-4 border">
                    <p class="text-gray-700"><strong>ID:</strong> 00{{ admin.id }}</p>
                    <p class="text-gray-700"><strong>ชื่อ:</strong> {{ admin.name }}</p>
                    <p class="text-gray-700"><strong>Username:</strong> {{ admin.username }}</p>
                    <p class="text-gray-700"><strong>Password:</strong> {{ admin.password }}</p>
